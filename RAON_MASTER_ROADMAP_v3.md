...
- [x] **9.9 스마트플랜 맛보기 및 3단계 뱃지 자동 전환 (2026-08-11)** 🟢
  - [x] **3단계 뱃지 자동 전환**: 등록 당일 맛보기(1단계) -> 다음날 09시 정밀 생성 가능(2단계) -> 정밀 생성 완료(3단계) 수식 탑재.
  - [x] **6:2:2 인기도 및 티어 가점 이식**: KTO 60% + Tmap 20% + KT 20% 인기도 및 명성 티어 보너스(1티어 100, 2티어 80)를 맛보기 명소 스코어링에 100% 동일 이식.
  - [x] **맛보기 카카오내비 100% 원천 차단**: 맛보기 생성/재생성(갱신) 시 카카오내비 API 호출 100% 격리 차단 (`isPreviewMode` Props 주입).
  - [x] **정밀 카카오내비 100% 작동**: 오전 9시 이후 정밀 스마트플랜 가동 시에는 경로 탐색 프로세스가 정상 기동되도록 완벽 분리.
