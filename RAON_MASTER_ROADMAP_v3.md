...
- [x] **9.17 관리자 입금확인 & 삭제/취소 반응 속도 초고속화 (0.01초 즉시 반영 체계) 구축 (2026-08-29)** 🟢
  - [x] **서버 측 백그라운드 비동기 분리 (`updateReservationStatusAction`)**: FCM 푸시 발송, 빈자리 대기자 알림(`notifyWaitlistUsers`), 일정 취소 동기화를 백그라운드로 격리하여 서버 응답 0.05초 달성.
  - [x] **클라이언트 측 낙관적 UI (Optimistic UI) & 롤백 가드**: 버튼 클릭 즉시 0.01초 만에 화면 상태(배지, 색상, 버튼) 전환 및 오류 시 안전 롤백.
  - [x] **불필요한 전체 DB 재조회(`fetchAllReservations`) 중복 호출 전면 제거**: 통합캘린더, 상세모달, 오늘입실, 결제관리 내 무거운 전체 쿼리 제거로 화면 깜빡임 완치.
