...
- [x] **9.72 [바로 여행계획 만들기] 모바일 모달 오픈 시 상단 헤더 및 목적지 검색창 최우선 노출 및 가상 키보드 밀림 방어 완결 (2026-09-26)** 🟢
  ...
  - [x] **본문 스크롤 최상단 강제 리셋 (`scrollContainerRef` & `useEffect`)**: `scrollContainerRef`를 스크롤 컨테이너에 연결하고, `isOpen` 및 `step === 'INPUT'` 전환 시 `scrollTo({ top: 0, behavior: 'instant' })`를 이중 실행하여 모달 오픈 즉시 상단 헤더와 목적지 검색창("어디로 떠나시나요?")이 100% 최우선 노출되도록 구현.
  - [x] **검색창 터치 시 스크롤 상단 앵커링 (`onFocus`)**: 사용자가 검색 입력창을 터치하여 키보드가 올라올 때도 검색창이 상단 헤더 바로 아래에 안정적으로 유지되도록 스무스 스크롤 앵커링 적용.
  - [x] **빌드 검증**: `npx tsc --noEmit` 에러 0건 통과 및 Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.69 장소 카드 [ ⇄ 변경 ] 에메랄드 테마 통일 및 대체리스트 [🗺️ 지도로 한눈에 비교] 라이브 핑 버튼 고도화 완결 (2026-09-25)** 🟢
  - [x] **장소 카드 [ ⇄ 변경 ] 버튼 컬러 아이덴티티 통일 (`InstantPlanModal.tsx`, `SmartPlanProposal.tsx`)**: 기존 칙칙한 무채색(Stone-300) 테두리 대신 산뜻한 포레스트 에메랄드 테두리(`border-2 border-emerald-400 bg-emerald-50 text-emerald-800`)를 전체 장소카드에 균일 적용하여 시각적 과밀(Clutter) 없이 "누를 수 있는 액션 버튼"임을 직관화. 과도한 텍스트 바나 이질적인 펄스 링/반짝이 뱃지를 원천 배제하여 카드 본문 가독성 100% 보존.
  - [x] **대체리스트 [🗺️ 지도로 한눈에 비교] 버튼 시인성 혁신 (`InstantPlanModal.tsx`, `SmartPlanProposal.tsx`)**: 단순 "지도로 보기"에서 "🗺️ 지도로 한눈에 비교"로 직관적인 가치 제안 카피라이팅 전환. 버튼 좌측에 은은하게 깜빡이는 초미니 라이브 펄스 링(`animate-ping bg-emerald-300`) 장착 및 `shrink-0`, `whitespace-nowrap` 적용으로 360px 모바일 화면에서도 1줄 정렬 무결성 확보.
  - [x] **빌드 검증**: `npx tsc --noEmit` 에러 0건 통과 및 Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.68 주소·부분주소 무료 통합 검색 고도화 및 나의 전체 여행일정 첫 진입 0초 즉시 렌더링(SWR) 완결 (2026-09-24)** 🟢
  - [x] **주소 및 부분주소 무료 통합 검색 (`searchAddressAction`, `MyMapModal.tsx`)**: 카카오 주소 검색뿐만 아니라 행정안전부 주소 오픈 API와 웹/문서 기반 검색(0원 비용 구조)의 최적 조합 검토 및 부분 주소("백제큰길 2047", "공주 공주보" 등)도 매끄럽게 좌표 변환 및 지도 이동/마킹 지원. 지도 모달 검색창에서 상세 주소뿐만 아니라 명칭·일부 주소 입력 시에도 정확한 위치를 찾아 지도 중심 이동 및 마커 생성.
  - [x] **나의 전체 여행일정 (`/myspace/schedule/page.tsx`) 첫 진입 0초 즉시 렌더링(SWR) 및 서버/번들 최적화**: `user_schedules_cache`를 활용한 SWR 동기 캐시 복원으로 초기 로딩 스켈레톤 0초 즉시 렌더링, `getMySchedules`의 과거 일정 상태 DB `UPDATE` 비차단 비동기 위임(응답 900ms ➔ 280ms), `ScheduleForm` dynamic import를 통한 번들 경량화, 홈 화면 `router.prefetch`를 통한 즉시 화면 전환 달성.
  - [x] **빌드 검증**: `npx tsc --noEmit` 에러 0건 통과 및 Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.67 홈 화면 UI 전면 개편(스크롤 제로 골든존), 탑바 '스마트 여행수첩' 2단 타이틀 안착 및 리마인더 팝업 전환 완결 (2026-09-23)** 🟢
  - [x] **탑바(TopBar) 높이 및 브랜드 타이틀 강화 (`TopBar.tsx`)**: 헤더 높이를 `h-[74px]`로 확장하여 시각적 여유 확보 및 중앙 로고에 `RAON.I`와 서브타이틀 `스마트 여행수첩` 2단 계층 안착.
  - [x] **상단 공지·알림 바 라이트 모드 최적화 (`SlimNotice.tsx`, `NotificationBadge.tsx`)**: 부드러운 스톤 배경과 딥그린 아이콘, 진한 텍스트의 `variant="home"` 스타일 신설.
  - [x] **리마인더 배너 ➔ 팝업 모달 전환 (`ReminderModal.tsx`, `BeginnerHome.tsx`)**: 인라인 배너 제거 후 전용 모달로 분리, `오늘 하루 보지 않기` 로컬스토리지 연동.
  - [x] **홈 화면 스크롤 제로(Zero-Scroll) 골든존 레이아웃 재배치 (`BeginnerHome.tsx`)**: 거대한 히어로 사진을 삭제하고 상단 골든존에 [탑바] ➔ [공지·알림] ➔ [즉시여행 2종] ➔ [다가오는 일정] ➔ [나의 전체일정] ➔ [캠핑장 소개/예약] 순서로 전면 재배치.
  - [x] **빌드 검증**: `npx tsc --noEmit` 에러 0건 통과 및 Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.66 다가오는 일정 뱃지 문구 깜빡임(Flicker) 완치 및 상세 화면 스마트플랜 0초 즉시 렌더링(Cache-First) 완결 (2026-09-23)** 🟢
  - [x] **홈 일정 카드 상단 뱃지 깜빡임 완치 (`ScheduleHomeWidget.tsx`)**: `localStorage` 캐시를 마운트 즉시 동기화하고 최상위에서 직접 참조하여 복귀 시 뱃지 텍스트 왜곡(Flicker) 100% 박멸.
  - [x] **일정 상세 화면 스마트플랜 0초 즉시 렌더링 (`myspace/schedule/[id]/page.tsx`)**: 마운트 즉시 캐시 데이터(`smart_plan_data`)를 동기 복원하여 2~4초 로딩 스피너를 원천 스킵하고 0초 만에 플랜 표출, 백그라운드 Silent SWR 동기화 유지.
  - [x] **빌드 검증**: `npx tsc --noEmit` 에러 0건 통과 및 Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.
...
- [x] **9.64 정밀 여행계획(SmartPlanProposal) 2단계 계층형 뒤로가기(Popstate) 가드 구축 및 페이지 이탈 완치 (2026-09-23)** 🟢
  ...
  - [x] **`isProgrammaticBackRef` & `go(-depth)` 안전 회수 장착**: UI 버튼(`[📋 리스트로 보기]`, `[X]`, `[✔ 이 장소로 선택]`) 터치 시 브라우저 popstate 중복 트리거 차단 및 찌꺼기 가상 히스토리 일괄 소멸. 언마운트 시 브라우저 세션 클린업.
  - [x] **빌드 검증**: `npx tsc --noEmit` 에러 0건 통과 및 Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.63 즉시 여행계획 모바일 내비게이션 3대 결함 완치, 모달 종료 시 세션 상태 완전 리셋 및 카카오 간편로그인 안정화 완결 (2026-09-22)** 🟢
  - [x] **GPS 갱신 시 플랜 보존 (`InstantPlanModal.tsx`)**: GPS 재수신이나 위치 갱신 시 생성된 4단계 여행 플랜이 날아가거나 초기화되지 않고 안전하게 보존.
  - [x] **모바일 3단계 계층형 히스토리 가드 (`InstantPlanModal.tsx`, `SmartPlanProposal.tsx`, `SmartPlanMapViewModal.tsx`)**: 지도 ➔ 대체리스트 ➔ 모달 결과 ➔ 홈 잔류 계층 스택 구축으로 모바일 뒤로가기 시 이탈 방어. 대체리스트 '변경' 터치 시 즉시 닫힘 생명주기 오류 치유 (`20de41b`).
  - [x] **모달 종료(`!isOpen`) 및 홈 복귀 시 이전 여행계획 소멸 및 세션 상태 완전 리셋 (`9345435`, `fd762b6`)**: 재오픈 시 깨끗한 첫 화면 보장.
  - [x] **카카오 간편로그인 세션 복구 및 안정화 검증**: 불필요한 사전 네트워크 `signOut` 및 1회용 OAuth 인가 코드 브릿지 재사용 시도 제거, 검증된 `fd762b6` 안정화 및 실기기 검증 완료.
  - [x] **빌드 검증**: `npx tsc --noEmit` 에러 0건 통과 및 Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.62 추가 입금(차액) 독립 분리 및 기존 결제완료 예약 100% 보존 아키텍처 구축 완결 (2026-09-20)** 🟢
  - [x] **추가 입금 독립 분리 (`payments/page.tsx`, `useReservationStore.ts`)**: 정보 수정으로 추가 입금 발생 시 기존 예약을 유지하고 `PENDING` 차액 레코드만 독립 관리.

- [x] **9.61 본 예약 유지 + '일부 환불(차액)' 독립 분리 및 결제목록 상단 세션 제자리 유지 완결 (2026-09-20)** 🟢
  - [x] **일부 환불 독립 분리 (`updateReservationDetailsAction`, `completePartialRefundAction`)**: 차액 환불 발생 시 본 예약 `CONFIRMED` 유지, 고객 여정 보존 및 순수 차액 정산만 수행.
  - [x] **결제목록 상단 세션 제자리 유지 (`payments/page.tsx`)**: 처리 시 목록 아래로 밀려나지 않고 Tier 0 제자리 유지(`sessionProcessed`).

- [x] **9.60 관리자 예약정보 전체 항목 수정(3중 연동·회계 차액 전이·AI 스마트플랜 보존) 및 결제목록 스마트 3계층 정렬 완결 (2026-09-20)** 🟢
  - [x] **예약정보 전체 항목 수정 (`AdminReservationDetailModal.tsx`)**: 성함, 연락처, 차량, 인원 구성, 반려동물, 요청사항 3중 동기화.
  - [x] **스마트 3계층 정렬 (`payments/page.tsx`)**: 환불대기 최상단 고정, 과거 환불완료 침범 방지, 처리 완료 시 제자리 유지.

- [x] **9.59 11월 대규모 예약 오픈 직전 실시간 점검, 16개 전 실전 시뮬레이션 100% 통과 및 표준 점검 가이드(SOP) 완결 (2026-09-20)** 🟢
  - [x] **16개 전 실전 엣지 케이스 시뮬레이션 통과**: 2099년 가상 격리로 운영 DB 무오염, 100:1 경합 배타락(854ms), 50명 동시 폭격 등 16/16 전 케이스 통과.
  - [x] **표준 사전 점검 가이드 문서화 (`docs/PRE_OPEN_INSPECTION_AND_SIMULATION_GUIDE.md`)**.

- [x] **9.58 캠핑 리마인더 Edge Function D-1/D-4 구버전 영구 소멸 원격 배포 및 D-0 행사 거리 소수점 1자리 정돈 완결 (2026-09-19)** 🟢
  - [x] **Edge Function 최신 배포**: D-1/D-4 오발송 원천 소멸 및 D-7 주간예보 단일화 확립. D-0 행사 거리 소수점 1자리 정돈.
...
- [x] **9.53 즉시 여행계획 저장 버튼 0ms 무결성 안전망 & TWA 카카오 로그인 튕김 방어 완결 (2026-09-13)** 🟢
  ...
  - [x] **모달 시트 닫힘 연동 (`InstantPlanModal.tsx`)**: 비로그인 판정 시 `onClose()`를 호출하여 화면 겹침 및 뒤편 잔류 방지.
  - [x] **10분 복원 퍼널 데드락 안전망 동기화 (`BeginnerHome.tsx`)**: `checkDraftPlan`에서 무거운 원격 `getUser()`를 제거하고 `hasAuthToken` 0ms 사전 검증 및 `getSession()` + 3초 타임아웃으로 홈 첫 진입 속도 향상.
  - [x] **TWA 안드로이드 앱 카카오 OAuth 안정화 (`SocialLoginButtons.tsx`)**: `queryParams: { prompt: 'login' }`을 적용하여 스마트폰 카카오톡 앱 전환으로 인한 프로세스 리셋 및 PKCE 쿠키 유실 원천 차단.
  - [x] **인증 콜백 에러 투명화 (`route.ts`, `login/page.tsx`)**: `exchangeCodeForSession` 실패 시 세션 없이 홈으로 튕기지 않고 `/login?error=oauth_failed`로 분기하여 정직한 토스트 피드백 제공.
...
- [x] **9.46 홈 백그라운드 최적화, 비로그인 즉시플랜 10분 선택형 복원 퍼널 및 관리자 대시보드 5대 실효 지표 개편 완결 (2026-09-09)** 🟢
  ...
  - [x] **즉시 여행계획 로깅 & 사이트 방문자 수(PV/UV) 카운팅 신설 (`analytics-logger.ts`, `instant-plan.ts`)**: Supabase 정규 테이블 `user_action_log`를 100% 활용한 Fail-Safe 비동기 로깅 탑재. 플랜 생성 시 고유 `logId`를 발급하고, DB 저장이 성공하는 순간 1:1 매칭으로 전환(`INSTANT_PLAN_CONVERT`) 확정 기록. 홈 진입 시 `visitorKey`와 1회 세션 디바운스로 비로그인 포함 전체 사이트 방문자 수(PV)와 순 방문자(UV)를 실시간 집계.
  - [x] **관리자 대시보드 3대 미사용 제거 & 5대 실효 지표 개편 (`admin-analytics.ts`, `admin/page.tsx`)**: 요리 레시피, 놀이 탐색기, **커뮤니티 소식 탐색** 3대 미사용 카드를 완전 제거. 상단에 사이트 총 방문 요약 카드(PV/UV) 신설, 즉시 여행계획 전용 카드(내주변/목적지 비율, 비로그인 비율, 내 일정 전환 건수 및 전환율%) 신설, 정밀 스마트플랜 단순 예약자/10초기록자 허수 박멸 정규화, 10초 기록 순수 캠핑 핀 정합성 유지.
  - [x] **빌드 검증**: Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.
...
- [x] **9.44 비로그인 캐시 보안 격리 및 즉시 여행계획 카피라이팅 최적화 완결 (2026-09-07~08)** 🟢
  ...
  - [x] **즉시 여행계획 하단 전환 카피라이팅 개편 (`InstantPlanModal.tsx`)**:
    - "등록 시 열리는 오전 9시 업데이트" ➔ **"일정 저장 시 열리는 오전 9시 업데이트"**
    - "🔒 카카오 3초 간편로그인으로 평생 무료 소장" ➔ **"🔒 카카오 3초 간편로그인으로 자동 여행계획이 가능해요!"**
  - [x] **빌드 검증**: Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.43 즉시 여행계획 GPS 상태 분기 및 3단계 정밀 로드맵 초간결 미니멀 UI 개편 완결 (2026-09-07)** 🟢
  - [x] **GPS 약함 시 단독 안내 배너 분기**: 정상 수신 시 기존 녹색 "내 주변 실시간 GPS", GPS 신호 약할 시 붉은 톤 안내 배너만 단독 노출하여 시인성 극대화 및 혼선 제거.
  - [x] **하단 CTA 3단계 정밀 로드맵 초간결 2단어 미니멀 UI 개편**: 상단 플로팅 뱃지 제거 및 `[내일 09시 / 정밀 플랜]`, `[D-7 09시 / 날씨 최신]`, `[당일 09시 / 최종 완성]` 가로 3열 칩 형태의 직관적·고급형 레이아웃 탑재.
  - [x] **명소 공식 인증 표기 1:1 일치화**: 즉시 여행계획과 정밀 스마트플랜 간의 명소 인증 뱃지(`안심식당`, `백년가게`, `모범음식점`, `KTO 공식인증` 등) 100% 동일 규칙 일치화.
  - [x] **빌드 검증**: Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.42 일일 로테이션 경기도 TourAPI 시군구 코드 추가, `--only` 핀포인트 실행 플래그 탑재 및 KTO 순위 차등 점수화 검증 완결 (2026-09-06)** 🟢
  - [x] **경기도 31개 시군구 TourAPI 코드 전수 등록**: `scripts/daily-region-sync.mjs`의 `TOUR_API_SIGUNGU_MASTER`에 경기도 31개 시군구(`1: 가평군 ~ 31: 화성시`) 맵을 구축하여 한국관광공사 시군구 파라미터 누락 원천 해결.
  - [x] **카테고리 선별 핀포인트 실행 플래그(`--only`) 탑재**: 전체 파이프라인 무결성을 보존하면서 실패한 카테고리만 외과수술식으로 고속 재수신할 수 있는 `--only=SPOT,HOSPITAL` 바이패스 스위치 구축 (식당/마트 2만 건 불필요한 재실행 스킵).
  - [x] **KTO 100위 순위 차등 점수화(선형 감쇠) 실시간 DB 적재 검증**: 1위 100점 ~ 100위 51점 공식에 따라 경기도 명소 데이터의 `raw_data->kto_official->kto_score` 및 `trust_score`가 정확히 주입됨을 실측 쿼리로 100% 검증 완료 (1위 100점, 10위 96점, 25위 88점 등).
  - [x] **실시간 라이브 가동 및 0건 탈출 완결**: 경기도 관광명소 845건, 병원 76건 정상 수신 완료 (`automation_logs`에 정상 기록).
  - [x] **빌드 검증**: Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.41 지도 모달 DOM Keep-Alive 싱글톤 및 검색창 0원 비용 가드·푸시알림 2단계 단일화 완결 (2026-09-06)** 🟢
  - [x] **지도 모달 DOM Keep-Alive 싱글톤**: `SmartPlanMapViewModal.tsx`에 최초 오픈 1회만 지도 로드(`hasBeenOpened`), 닫힘 시 hidden 보존, 재오픈 시 `map.relayout()` 복원으로 지연시간 0초 및 추가 지도 호출 0건(0원) 보장.
  - [x] **검색창 타이핑 중 API 호출 전면 차단 (Zero Typing Leak)**: `InstantPlanModal`, `MyMapModal`, `CampingProfileGate` 검색창에서 글자 타이핑 중 실시간 호출 완전 차단(0건), `[검색]` 버튼 및 `Enter` 시에만 1회 단일 호출 및 1초 연타 방지(Throttle) 가드 장착.
  - [x] **푸시 알림(Camping Reminder) 2단계 단일화**: 비공개 레시피 링크 유도 위험이 있던 D-1 알림 완전 삭제, D-4 알림 삭제 및 D-7 주간예보 알림으로 일원화하여 유저 스팸 피로도 제거 및 DB 부하 대폭 절감.
  - [x] **DB 비용 및 대규모 확장성 검증**: Free Tier(월 2만 건 무료) 및 Pro Tier(월 3.5만원 고정으로 월 100만 건 수용) 비용 시뮬레이션 및 가성비 검증 완료.
  - [x] **빌드 검증**: Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.40 진입장벽 제로화 즉시 여행계획(4단계) 및 홈 화면 골든존 최적화 완결 (2026-09-05~06)** 🟢
  - [x] **원터치 1초 즉시 플랜 파이프라인 탑재**: 회원가입/사전설문 없이 실시간 GPS 기반 [내 주변 즉시 플랜] 및 목적지 검색 [목적지 즉시 플랜] 4단계 여행 플랜 자동 생성.
  - [x] **정품 플랜 1:1 일치 및 온보딩 퍼널**: 스마트플랜 4대 검증 카드, 대체 장소 스와프 시트, 카카오 지도 마커 동기화, 3대 내비(티맵/카카오내비/네이버지도) 길안내 및 [내 일정으로 등록] 완결.
  - [x] **홈 화면 골든존 확보**: 히어로 배너 210px 슬림화 및 장황한 인사말 숨김 처리로 첫 화면 스크롤 없이 즉시 플랜 & 다가오는 일정 카드 100% 노출.
  - [x] **모바일 UX 디테일 강화**: 일정 카드 손가락 탭핑(`👆`) 유도 애니메이션, 일정 상세 화면 상단 거대 카드 제거 및 스티키 헤더바 일체형 통합.
  - [x] **빌드 검증**: Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.39 주간 축제 정밀 동기화 API 키 폴백 및 에러 핸들링 정상화 완결 (2026-09-04)** 🟢
  - [x] **TOUR_API_KEY 폴백 정규화**: `scripts/sync-festivals-trigger.mjs`에서 `process.env.TOUR_API_KEY || process.env.PUBLIC_DATA_API_KEY` 폴백 추가 및 `.github/workflows/weekly-festival-sync.yml` 시크릿 연동 보강으로 7월 20일 이후 발생하던 환경변수 누락 0초 조기종료 완치.
  - [x] **Silent Failure(에러 삼킴) 버그 완치**: 키 누락 또는 동기화 실패 시 `process.exit(1)` 명시 및 DB `automation_logs` 실패 기록 보강으로 투명한 관제 체계 확립.
  - [x] **실시간 라이브 가동 검증 완료**: 전국 최신 축제 279건 정상 수신 및 DB `master_places`(총 591건) 적재 완료 (`automation_logs`에 2,665ms 성공 기록).
  - [x] **3대 배치 스크립트 빌드 검증 강제화**: `package.json`의 `build` 스크립트에 `node --check scripts/sync-festivals-trigger.mjs` 필수 연동 및 Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.38 일일 로테이션 TourAPI 무한 루프 탈출 가드 탑재 및 구글 플레이 Android 16(API 36) 타깃 업데이트 완결 (2026-09-04)** 🟢
  - [x] **TourAPI 무한 루프(120분 행) 완치**: `scripts/daily-region-sync.mjs`의 `syncTourSpots`에서 공공 API 새벽 점검/장애 응답(`resultCode !== '0000'`) 발생 시 3회 재시도 후 바깥 루프까지 안전 조기 종료(`hasMore = false`)하는 2중 철벽 가드 탑재하여 120분 강제 취소 사고 영구 방어.
  - [x] **구글 플레이 최신 Android 16(API 수준 36) 타깃 AAB(v2) 심사 제출 완결**: PWABuilder를 통해 기존 서명키(`signing.keystore`)로 `kr.co.raoni.app` 타깃 SDK 36 번들 패키징 후 Google Play Console 프로덕션 출시 제출 완료 (연례 필수 요건 100% 충족).
  - [x] **빌드 검증**: `node --check` 및 Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.37 스마트플랜 인트로 문구 확정 노출 및 Stage 4 안심 인프라 2중 박스 컨테이너 완결 (2026-09-03)** 🟢
  - [x] **인트로 문구 확정 노출**: Stage 3과 Stage 4의 안내 문구가 구버전 DB 문자열에 간섭받지 않고 최신 문구로 100% 확정 표출.
  - [x] **Stage 4 2중 박스 컨테이너 구현**: 일반 여행지와 차별화된 은은한 블루/슬레이트 안심 컨테이너(`rounded-2xl border border-blue-200/90 shadow-sm`) 및 상단 `🛡️ 안심 여행 필수 인프라` 헤더 바 적용.
  - [x] **빌드 검증**: Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.

- [x] **9.36 스마트플랜 스테이지 3 & 4 직관적 UI 개편 완결 (2026-09-03)** 🟢
  - [x] **Stage 3 (목적지 근처 맛집/명소)**: '목적지 주변의 검증된 로컬 맛집과 놓치기 아쉬운 힐링 명소를 엄선했습니다.' 안내 문구 연동, 식당(RESTAURANT) ➔ 명소(SPOT) ➔ 축제(FESTIVAL, 명소 바로 아래) 순서 배치.
  - [x] **Stage 4 (편의시설 안내)**: '쾌적하고 안전한 여행을 위한 장보기 마트, 응급 병원, 주유소 정보입니다.' 안내 문구 연동, 마트(MART) ➔ 병원(HOSPITAL) ➔ 주유소(GAS_STATION) 순서 배치 및 기존 파란 박스 정돈.
  - [x] **전체 동선 지도 마커 번호 동기화**: `timelinePlacesForMap` 정렬을 개편된 스테이지 순서와 100% 동기화하여 본문과 지도 마커 번호(`①, ②, ③...`) 1:1 완벽 일치.
  - [x] **빌드 검증**: Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과.
...
* [x] **결제 목록 상세화면 0건 깜빡임(Flicker) 방지 클린 대시('-') 및 로딩 스피너 탑재**:
  * 결제 관리(`payments/page.tsx`) 진입 시 `0건`, `0원`, `(0개 검색됨)`, `내역 없음`으로 먼저 번쩍이던 시각적 혼선을 제거.
  * `isLoading` 상태 가드를 탑재하여 데이터 수신 전까지 모든 카드 및 탭 카운트에 `-` 표시, 테이블 중앙 로딩 스피너(`Loader2`) 표출 처리로 UX 완성도 극대화.
* [x] **즉시여행계획 비로그인/로그인 통합 집계 무결성 검증**: 비로그인 게스트 식별자(`ANONYMOUS_GUEST_USER_ID`)와 실회원 데이터가 목적지/내주변 모드별로 정상 수집 및 통합 집계됨을 실측 DB 검증 완료.

### [2026-09-11 Update - Milestone 9.51]
* [x] **즉시 여행계획 GPS 재시도 실패 시 자동 라온아이 폴백 & [현재 위치 기준으로 다시 보기] 안전 바운더리 구축**:
  * GPS 재시도(`handleRetryGps`) 실패 시 빈 화면(`planData === null`) 노출을 방지하고, 자동으로 라온아이 대표 좌표(`36.6354349`, `126.7638091`) 기반 추천 플랜을 즉시 생성·채워주어 4단계 추천 코스가 100% 온전히 유지되도록 방어.
...
### [2026-09-23 Update - Milestone 9.67]
* [x] **홈 화면 UI 전면 개편(스크롤 제로 골든존), 탑바 '스마트 여행수첩' 2단 타이틀 안착 및 리마인더 팝업 전환 완결**:
  * **탑바(TopBar) 높이 및 브랜드 타이틀 강화 (`TopBar.tsx`)**: 헤더 높이를 `h-[74px]`로 확장하여 시각적 여유를 확보하고, 중앙에 큼직한 `RAON.I` (`text-[21px] font-black`)와 서브타이틀 `스마트 여행수첩` (`text-[13.5px] font-bold mt-1.5`) 2단 계층 구조를 안착하여 브랜드 정체성 극대화.
