...
function getSidoShort(sido) {
    if (!sido) return '';
    if (sido.endsWith('권')) return ''; // Ignore '수도권', '강원권' etc.
    return sido.substring(0, 2);
}

async function absoluteFinalAuditV2() {
    console.log('--- [Absolute Final Audit v2] Region Name Fixed ---');
    ...
    uniquePrestige.forEach(p => {
        ...
        const pSidoShort = getSidoShort(p.sido);
        const pSigunguShort = getSidoShort(p.sigungu); // Use sigungu as backup for sido short
        const effectiveSido = pSidoShort || pSigunguShort;
        ...
        let match = allMasterSpots.find(m => {
            ...
            // Rule A: Sido must match if both have real Sido info
            if (effectiveSido && mSidoShort && effectiveSido !== mSidoShort) return false;

            // Rule B: Sigungu check
    ...
    fs.writeFileSync('absolute_final_audit_report_v2.json', JSON.stringify(report, null, 2));
    
    console.log(`\n--- [Absolute Final Audit v2 Summary] ---`);
...
absoluteFinalAuditV2().catch(console.error);
