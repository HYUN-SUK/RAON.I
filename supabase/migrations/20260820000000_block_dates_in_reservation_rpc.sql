-- =====================================================
-- create_reservation_safe RPC에 blocked_dates(관리자 차단/대관) 검사 추가
-- 2026-08-20
-- 
-- 목적: 관리자가 blocked_dates에 등록한 날짜(예: 넥슨 전체대관 등)에
--       어떤 클라이언트 에러나 우회 요청이 있더라도 DB 레벨에서 100% 원천 차단
-- =====================================================
...
BEGIN
    ...
    -- 3-2. 관리자 차단일/대관(blocked_dates) 중복 검사 (철벽 방어 추가)
    SELECT COUNT(*) INTO v_existing_count
    FROM blocked_dates
