...
DROP POLICY IF EXISTS "Users can view their own consents" ON public.user_permission_consents;
CREATE POLICY "Users can view their own consents"
ON public.user_permission_consents FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can upsert their own consents" ON public.user_permission_consents;
CREATE POLICY "Users can upsert their own consents"
ON public.user_permission_consents FOR ALL
USING (auth.uid() = user_id);

-- 관리자 정책: 전체 조회 가능 (집계용)
DROP POLICY IF EXISTS "Admins can view all consents" ON public.user_permission_consents;
...
DROP FUNCTION IF EXISTS update_user_permission_consents_updated_at() CASCADE;
...
DROP TRIGGER IF EXISTS trigger_update_user_permission_consents_updated_at ON public.user_permission_consents;
...
SELECT DISTINCT ON (user_id) user_id, true, created_at
FROM push_tokens
