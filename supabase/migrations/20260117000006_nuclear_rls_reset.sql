...
DROP POLICY IF EXISTS "policy_reservations_select_v3" ON public.reservations;
...
DROP POLICY IF EXISTS "policy_reservations_insert_v3" ON public.reservations;
CREATE POLICY "policy_reservations_insert_v3" ON public.reservations
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

-- 3. [예약] 수정 (UPDATE): 본인 것 OR 관리자
DROP POLICY IF EXISTS "policy_reservations_update_v3" ON public.reservations;
...
DROP POLICY IF EXISTS "policy_notifications_select_v3" ON public.notifications;
...
DROP POLICY IF EXISTS "policy_notifications_insert_v3" ON public.notifications;
CREATE POLICY "policy_notifications_insert_v3" ON public.notifications
    FOR INSERT WITH CHECK (true);

-- 6. [알림] 수정 (UPDATE): 본인 것 (읽음 처리 등)
DROP POLICY IF EXISTS "policy_notifications_update_v3" ON public.notifications;
