...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'point_history' AND column_name = 'xp_amount') THEN
        ALTER TABLE public.point_history ADD COLUMN IF NOT EXISTS xp_amount INTEGER DEFAULT 0;
    END IF;
END $$;
...
DROP FUNCTION IF EXISTS sync_mission_like_to_comment() CASCADE;
...
DROP FUNCTION IF EXISTS sync_comment_like_to_mission() CASCADE;
