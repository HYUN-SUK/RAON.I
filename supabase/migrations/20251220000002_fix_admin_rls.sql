...
DROP POLICY IF EXISTS "Enable all access for authenticated users (Dev)" ON creator_contents;
...
DROP POLICY IF EXISTS "Enable all access for authenticated users (Dev)" ON creator_episodes;
create policy "Enable all access for authenticated users (Dev)"
on creator_episodes
for all
to authenticated
