...
CREATE INDEX IF NOT EXISTS idx_open_day_rules_active ON public.open_day_rules(is_active) WHERE is_active = true;
...
DROP POLICY IF EXISTS "Public can read active rules" ON public.open_day_rules;
...
DROP POLICY IF EXISTS "Admins can do everything" ON public.open_day_rules;
...
DROP FUNCTION IF EXISTS maintain_single_active_rule() CASCADE;
...
DROP TRIGGER IF EXISTS trigger_single_active_rule ON public.open_day_rules;
