...
-- Likes
DROP POLICY IF EXISTS "Likes are viewable by everyone" ON creator_content_likes;
CREATE POLICY "Likes are viewable by everyone" ON creator_content_likes
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Users can like contents" ON creator_content_likes;
CREATE POLICY "Users can like contents" ON creator_content_likes
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can unlike contents" ON creator_content_likes;
CREATE POLICY "Users can unlike contents" ON creator_content_likes
  FOR DELETE USING (auth.uid() = user_id);

-- Comments
DROP POLICY IF EXISTS "Comments are viewable by everyone" ON creator_content_comments;
CREATE POLICY "Comments are viewable by everyone" ON creator_content_comments
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Users can create comments" ON creator_content_comments;
CREATE POLICY "Users can create comments" ON creator_content_comments
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own comments" ON creator_content_comments;
CREATE POLICY "Users can delete their own comments" ON creator_content_comments
  FOR DELETE USING (auth.uid() = user_id);

-- Follows
DROP POLICY IF EXISTS "Follows are viewable by everyone" ON creator_follows;
...
DROP POLICY IF EXISTS "Users can follow creators" ON creator_follows;
CREATE POLICY "Users can follow creators" ON creator_follows
  FOR INSERT WITH CHECK (auth.uid() = follower_id);

DROP POLICY IF EXISTS "Users can unfollow creators" ON creator_follows;
...
DROP FUNCTION IF EXISTS update_creator_content_like_count() CASCADE;
...
DROP TRIGGER IF EXISTS trigger_update_creator_content_like_count ON creator_content_likes;
...
DROP FUNCTION IF EXISTS update_creator_content_comment_count() CASCADE;
...
DROP TRIGGER IF EXISTS trigger_update_creator_content_comment_count ON creator_content_comments;
...
DROP FUNCTION IF EXISTS update_creator_follower_count() CASCADE;
...
DROP TRIGGER IF EXISTS trigger_update_creator_follower_count ON creator_follows;
