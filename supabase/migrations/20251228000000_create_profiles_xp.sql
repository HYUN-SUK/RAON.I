...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'raon_token') THEN
        ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS raon_token INTEGER DEFAULT 0;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'gold_point') THEN
        ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS gold_point INTEGER DEFAULT 0;
    END IF;
END $$;
...
DROP FUNCTION IF EXISTS grant_user_reward(uuid, integer, integer, text, uuid) CASCADE; -- Old Sig
DROP FUNCTION IF EXISTS grant_user_reward(uuid, integer, integer, integer, text, uuid) CASCADE; -- New Sig (Safe Check)
DROP FUNCTION IF EXISTS use_user_point(uuid, integer, text) CASCADE; -- Old Sig
