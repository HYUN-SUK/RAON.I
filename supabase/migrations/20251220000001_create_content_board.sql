...
DROP POLICY IF EXISTS "Public creators are viewable by everyone" ON creators;
...
DROP POLICY IF EXISTS "Published contents are viewable by everyone" ON creator_contents;
...
DROP POLICY IF EXISTS "Creators can view their own contents" ON creator_contents;
CREATE POLICY "Creators can view their own contents" ON creator_contents
  FOR SELECT USING (auth.uid() = creator_id);

-- Creators can insert their own contents
DROP POLICY IF EXISTS "Creators can insert their own contents" ON creator_contents;
CREATE POLICY "Creators can insert their own contents" ON creator_contents
  FOR INSERT WITH CHECK (auth.uid() = creator_id);

-- Creators can update their own contents
DROP POLICY IF EXISTS "Creators can update their own contents" ON creator_contents;
...
DROP POLICY IF EXISTS "Published episodes are viewable by everyone" ON creator_episodes;
...
DROP POLICY IF EXISTS "Creators can view their own episodes" ON creator_episodes;
...
DROP POLICY IF EXISTS "Creators can insert their own episodes" ON creator_episodes;
...
DROP POLICY IF EXISTS "Creators can update their own episodes" ON creator_episodes;
...
CREATE TRIGGER update_creators_updated_at BEFORE UPDATE ON creators FOR EACH ROW EXECUTE PROCEDURE update_updated_at_column();
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_creator_contents_updated_at') THEN
        DROP TRIGGER IF EXISTS update_creator_contents_updated_at ON creator_contents;
CREATE TRIGGER update_creator_contents_updated_at BEFORE UPDATE ON creator_contents FOR EACH ROW EXECUTE PROCEDURE update_updated_at_column();
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_creator_episodes_updated_at') THEN
        DROP TRIGGER IF EXISTS update_creator_episodes_updated_at ON creator_episodes;
