...
-- 4. RLS 정책
-- 사용자는 자신의 배지만 조회 가능
DROP POLICY IF EXISTS "Users can view own badges" ON public.in_app_badges;
create policy "Users can view own badges"
on public.in_app_badges for select 
using (auth.uid() = user_id);

-- 사용자는 자신의 배지만 읽음 처리 가능
DROP POLICY IF EXISTS "Users can update own badges" ON public.in_app_badges;
create policy "Users can update own badges"
on public.in_app_badges for update 
using (auth.uid() = user_id);

-- 시스템/관리자가 배지 생성 가능 (서비스 역할)
DROP POLICY IF EXISTS "Service can insert badges" ON public.in_app_badges;
create policy "Service can insert badges"
on public.in_app_badges for insert 
with check (true);

-- 5. 배지 개수 조회 함수 (탭별)
DROP FUNCTION IF EXISTS get_badge_counts(uuid) CASCADE;
...
-- 6. 특정 탭 배지 읽음 처리 함수
DROP FUNCTION IF EXISTS mark_badges_as_read(uuid, text) CASCADE;
