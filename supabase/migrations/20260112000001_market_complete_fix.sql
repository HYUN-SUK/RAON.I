...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'site_config' AND column_name = 'market_categories') THEN
        ALTER TABLE site_config ADD COLUMN IF NOT EXISTS market_categories JSONB DEFAULT '[
...
CREATE POLICY "Public Read Access for product_images"
    ...
    IF NOT EXISTS (
        ...
        DROP POLICY IF EXISTS "Authenticated Upload for product_images" ON storage.objects;
