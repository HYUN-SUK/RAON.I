...
### 🟢 마일스톤 9.61: 본 예약 유지 + '일부 환불(차액)' 독립 분리 및 결제목록 상단 세션 제자리 유지 완결 (2026-09-20)

1. **11월 20일 순이네 김범주님 예약 복원 및 임시 차단 해제**:
   - 관리자가 사이트 보호를 위해 수동 등록했던 차단(`blocked_dates` - `05b63b9c...`) 삭제 완료.
   - 김범주님 예약(`f95df402...`)을 `CONFIRMED`(예약 확정) 상태로 복원 완료.
   - 함께 취소되었던 캠핑 여행 일정(`user_schedules`)을 `scheduled`(정상 진행 중) 상태로 원상 복구 완료.
   - 통합캘린더 확인 시 11/20 순이네 사이트가 정상 점유로 채워짐 확인 완료.

2. **정보 수정으로 인한 '일부 환불(차액)' 독립 분리 아키텍처 구축**:
   - **`updateReservationDetailsAction`**: 차액 환불 발생(`diff < 0`) 시 본 예약 `status`는 무조건 **`CONFIRMED` 유지** (사이트 점유 100% 보존, 오버부킹 원천 차단, 고객 마이스페이스 예약확정 유지). `refund_amount = Math.abs(diff)` 및 `refunded_at = null`로 일부 환불 대기 상태 관리.
   - **`completePartialRefundAction`**: 신설. 전체 취소 함수(`completeRefund`)와 달리 고객 여정 취소(`cancelScheduleByReservation`)를 절대 호출하지 않고 **순수 차액 정산(`refunded_at` 기록)만 수행**.
   - **`useReservationStore.ts`**: `completePartialRefund` 스토어 액션 탑재 및 `refundedAt` 매핑 완료.

3. **결제목록(`payments/page.tsx`) '일부환불' 독립 행 노출 및 상단 세션 제자리 유지 (`sessionProcessed`)**:
   - **`PaymentListItem` 가상 분리 모델 도입**: 본 예약 행(예: 김범주 - 결제완료 160,000원)과 `[일부환불]` 행(김범주 - 일부환불 대기 -30,000원)을 독립적으로 렌더링.
   - **상단 세션 제자리 유지 (`sessionProcessed`)**: 관리자가 화면에서 [환불완료] 또는 [입금확인]을 누르면, 화면이 재정렬되면서 행이 아래로 날아가지 않고 **Tier 0(최상단) 그 위치에 그대로 고정 유지**되어 관리자가 눈앞에서 처리 결과를 즉시 확인 가능.
   - **과거 환불 건 침범 방지**: 과거 8월에 완료된 9건의 환불완료 건은 신청일시 순(Tier 1)에 머물러 최신 9월 신규 예약을 가리지 않음.
   - **회계 집계 일치**: 상단 요약 카드의 '환불 대기 총액'에 일부 환불 대기 차액이 정확히 합산되어 표시.

4. **프로덕션 빌드 무결성 검증**:
   - `npm run build` Next.js 16.1.1 Production Build 103/103 전체 라우트 100% 정상 통과 (`Exit Code 0`).
