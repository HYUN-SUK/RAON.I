...
- **화면 맨 밑 상시 고정 바 (`fixed bottom-0`, `z-50`) (`verify/[scheduleId]/page.tsx`)**:
  - 팩트체크 화면 진입 시 하단 탭 메뉴(`BottomNav`)를 자동 숨김 처리하여 화면 공간을 100% 확보하고,
  - `[ 🌟 {N}곳 내 지도에 담고 +100P 받기 ]` / `[ 선택 없이 다음으로 넘어가기 → ]` 버튼이 화면 최하단에 시원하게 상시 노출되어 원클릭 처리 가능.
- **우측 상단 '다음에 하기 ✕' 칩 버튼 신설**:
  - 눈에 잘 띄는 선명한 둥근 칩 버튼으로 상단 우측에 상시 노출하여 언제든 0초 만에 부담 없이 이탈 가능.

### 1-6. 내 수첩 지도 오픈 & 맛집/명소 핀 영구 등록 및 카테고리 정합성 회복
- **내 수첩 `MyMapModal` 바인딩 (`myspace/page.tsx`)**:
  - 내 수첩 화면에 누락되어 있던 `MyMapModal`을 마운트하여 내수첩 10초 기록 후 [지도에서 핀 확인하기] 시 0초 만에 지도가 시원하게 열리도록 완결.
