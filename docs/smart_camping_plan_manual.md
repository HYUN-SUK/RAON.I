...
*   **Step 4. Phase 12: Real-time Verification (Track B - 가는 길 팩트)**: 중간지점(Midpoint) 반경 내에서 [식당], [카페], [명소]를 조회합니다. 
    ...
    *   **특수 항목(HOSPITAL) 실시간 연계**: 국립중앙의료원(NMC) API를 통해 실시간 응급실 가용 병상 정보를 수집하며, 수집된 데이터를 카카오 로컬 API로 2차 검증(별점, 리뷰 수 수합)하여 신뢰도를 보강합니다.
...
4. **동적 데이터의 영구 자산화 및 Upsert 체계 (2026-03-18 업데이트)**
   - **TTL 폐기**: 기존의 `4일 보존(TTL) 후 삭제` 방식을 완전히 폐기했습니다. D-3에 수집된 병원, 주유소, 축제 데이터는 이제 라온아이의 영구 자산으로 보존됩니다.
   - **결정론적 ID (UUID v5)**: 동적 데이터 수집 시에도 `api_source + 상호 + 주소`를 기반으로 한 UUID v5를 생성합니다. 이를 통해 동일 장소에 대해 중복 없이 정보를 최신화(Upsert)하는 하이브리드 동기화 체계를 완성했습니다.
...
## 🏗️ 6. 하이브리드 마스터 DB 동기화 시스템 (Phase 12: Persistent Hybrid Engine)
