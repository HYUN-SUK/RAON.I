...
*   **Step 3. Phase 11: Hybrid Master DB Scan (Track A - 현지 팩트)**: 캠핑장 반경 내 [식당], [마트], [명소], [주유소] 등은 Supabase 내부 `master_places`에서 PostGIS로 고속 선별합니다. 
    ...
    *   **1차 선별 및 중복 전략**: 동일 장소가 여러 공공 API(모범음심점, 안심식당, 백년가게 등)에서 중복 수집된 경우, **`api_source`를 포함한 결정론적 ID(UUID v5)**를 통해 각각 별도 레코드로 저장됩니다. 이는 향후 다중 인증 장소에 대한 신뢰 가중치 부여의 근거가 되며, **신뢰도 엔진(`reliability.ts`)에서 다음과 같은 중복 인증 보너스가 합산**됩니다:
        - **1개 인증**: 보너스 없음 (Standard)
        - **2개 인증 중복**: **+15점** 보너스 (예: 모범 + 안심)
        - **3개 이상 인증 중복**: **+30점** 보너스 (최고 신뢰 등급 - v10.0 고도화)
    *   **전국 데이터 소스 확장 (v10.0)**: 마트 카테고리의 실효성 확보를 위해 기존 '대규모점포(08_25_01)' 외에 **'준대규모점포(08_24_01)'**, **'기타 식품 판매업(06_07_01)'** 전국 소스를 통합 동기화합니다. ZIP 파일 내의 **모든 지역별(시도별) CSV를 순회하며 적재**하는 전국 단위 멀티 처리 엔진을 가동합니다.
...
- **계층형(Hierarchy) 스코어링 및 필터링 (v10.0 고도화)**: 
    1. **Tier 1 (100점)**: NMC 인증 응급의료기관, 권역센터, 종합병원, 의료원.
    2. **Tier 2 (70점)**: 일반 의원 (내과, 소아과, 외과, 가정의학 등 실제 캠핑 중 이용 가능 과목).
    3. **Tier 3 (50점)**: 공공 보건소 및 보건지소.
    4. **Safety Filter (원천제외)**: 정신병원, 동물병원, 보건행정관, 성형외과, 피부과, 요양병원 등 캠핑 중 긴급 의료와 무관한 시설 제외.
- **거리 가중치**: 위 계층별 점수에 거리 기반 Logistics 점수(반경 30km 확장)를 합산하여 최종 상위 15개를 선별합니다.

### 4.2 대형마트 및 지역 거점 마트 - `[MART]`
- **데이터 소스**: (1순위) LOCALDATA 준대규모점포(매장면적 3,000㎡ 미만), (2순위) 대규모점포(3,000㎡ 이상), (3순위) 슈퍼마켓/기타식품판매.
- **브랜드 우선순위 스코어링 (v10.0 고도화)**:
    1. **🥇 하나로마트/NH (90점)**: 지역 농지 거점 및 식재료 구비 강점.
    2. **🥈 대형 3사 (80점)**: 이마트, 롯데마트, 홈플러스, 노브랜드, 트레이더스.
    3. **🥉 SSM/중형 (65점)**: 식자재마트, 이마트에브리데이, 홈플러스익스프레스, 노브랜드.
- **Noise Filter**: 캠핑 식재료와 무관한 '패션아울렛', '의류타운', '슈즈/가전/가구 전문점' 원천 제외.
- **Supper Logic**: 일요일 포함 일정에 대형마트 휴무 위험(Risk Penalty -15)을 적용하며, 하나로마트에는 Diversity Bonus(+5)를 부여합니다.
- **점수(Score)**: 가중치 `[E:0.40, Q:0.10, CF:0.25, L:0.25]`. 존재 확실성(Existence) 최우선. 영유아 동반 시 '소아/아동' 키워드가 `persona_match=45`로 반영.

### 4.3 우수 식당 (다중 인증 보너스 적용) - `[RESTAURANT]`
- **데이터 소스**: (1순위) 행안부 모범음식점정보, (2순위) 농림축산부 안심식당, (3순위) 소진공 백년가게(SMBA_BAEK), (4순위) 카카오 높은평점.
- **다중 인증 신뢰도 보너스 (v10.0 고도화)**: 
    1. **3개 API 중복 인증**: **100점** (Base 70 + Bonus 30) - 최고 신뢰 등급.
    2. **2개 API 중복 인증**: **85점** (Base 70 + Bonus 15).
    3. **정기 공공 인증 (단일)**: 70점. 일반 식당: 60점.
- **Noise Filter (v10.0)**: 백년가게 중 식료품과 무관한 '안경', '의상', '한복', '건축' 등 비음식점 업종 원천 제외.
- **1차 선별**: 위 신뢰도 점수를 기준으로 정렬하되, 동일 점수 시 거리 기반 Logistics 점수를 합산하여 최종 20개를 선별합니다.
...
5. **Upsert Conflict Guard (Error 21000 방어)**: 
    - **장애 요인**: 수집된 배열 내부에 동일한 ID(`UUID v5`)가 2개 이상 존재할 경우, Supabase의 `.upsert()`는 SQL 규칙에 따라 배열 전체의 적재를 거부(Reject)합니다. (현상: 적재량 0건)
    - **방어 로직**: DB 적재 직전 Javascript의 `reduce`를 활용한 **Unique Filter**를 통과시켜 중복 행을 사전에 하나로 병합합니다.
6. **Audit & Telemetry (파이프라인 모니터링)**:
    - **로그 기록**: `automation_logs` 테이블을 통해 [수집된 총량] -> [카테고리별 필터 통과량] -> [최종 적재량]을 단계별로 추적합니다. 
