...
*   **Step 3. Phase 11: Hybrid Master DB Scan (Track A - 현지 팩트)**: 캠핑장 반경 내 [식당], [마트], [명소], [주유소] 등은 Supabase 내부 `master_places`에서 PostGIS로 고속 선별합니다. 
    ...
    *   **1차 선별 로직 (v9.3 개선)**: `get_master_places_in_radius` 호출 및 동적 수집본을 병합하여 카카오 검증 전 상한치(10~20개)를 추려냅니다. 이때 **날씨/성향 등 미래 변수는 일절 개입하지 않고**, 오직 오프라인 팩트 지표(마트: 면적/브랜드+거리, 병원/명소/축제: 거리 우선, 주유소: 가격우선+거리, 식당: 신뢰도우선+거리)만으로 정렬하여 누락 방지형 정예 후보군을 확보(recall)합니다.
...
*   **Step 4. Phase 12: Real-time Verification (Track B - 가는 길 팩트)**: 중간지점(Midpoint) 반경 내에서 [식당], [카페], [명소]를 조회합니다. 
    ...
    *   **특수 항목(HOSPITAL) 실시간 갱신 & 반경 검색**: 국립중앙의료원(NMC) API로 실시간 수집하여 `master_places`에 갱신 후, 행정구역 경계선 문제를 피하기 위해 **PostGIS 반경 15~30km 검색**으로 1차 후보를 추출합니다. 이후 카카오 2차 검증으로 신뢰도를 보강합니다.
...
- **데이터 소스**: (1순위) 국립중앙의료원 응급의료기관 API (실시간), (2순위) 마스터 DB 내의 보건소/의원/약국 정보.
- **1차 선별 (v9.3)**: 행정구역 경계를 무시하기 위해 API 실시간 호출 대신, 수집된 전체 병원 중 PostGIS 기준 가장 가까운 최단거리순으로 15개를 추출하여 카카오 검증 진입.
- **검색 계층 로직**: 
    1. **응급기관 우선**: 15km 반경 내 응급의료기관(`NMC_HOSPITAL`)을 먼저 탐색합니다.
    2. **반경 확장**: 15km 내에 없을 경우, 30km까지 5km 단위로 점진적으로 반경을 넓히며 탐색합니다.
    3. **내부 DB Fallback**: 30km 내에도 응급기관이 없을 경우, 마스터 DB의 보건소, 일반 병원, 의원을 후보로 채택합니다.
    4. **AI 119 안내**: 모든 의료시설 부재 시 AI 서사를 통해 119 구급대 이용 혹은 가장 가까운 시내 이동을 안내합니다.
...
- **수집 전략**: 주간 배치(정적)에서 제외하고, 캠핑 3일 전(D-3) 해당 캠핑장 중심 반경 10km 이상(중심+동서남북 다중 호출)의 등유 취급 주유소를 동적으로 수집하여 `master_places` 원본 저장 및 `smart_plan_facts`에 캐싱/영구 보존합니다.
- **1차 선별 (v9.3)**: 반경 10km 이상 오피넷 다중 수집 원문 중, 최저가순(K_PRICE)을 1순위로, 최단거리를 2순위로 정렬하여 10개를 선별.
...
4. **듀얼 DB 통합 영구 보존 정책 (Dual-Persistence)**
   - **`master_places` 유기적 증식**: D-3 크론잡이 실시간 수집한 병원, 주유소, 축제 API 원문 데이터(선별 전 100개, 50개 전체)를 `master_places`에 통째로 Upsert 합니다. 주간 배치가 미치지 못하는 사각지대를 캠핑장 중심으로 자동 증식 및 최신화합니다.
   - **`smart_plan_facts` 정예화 누적 (TTL 폐기)**: 기존의 `4일 보존(TTL) 후 삭제` 방식을 완전히 폐기했습니다. 1차 선별과 카카오 품질 검증까지 무사히 마친 10~20개의 고급 데이터들은 `smart_plan_facts`에 Upsert되어 최고급 에셋으로 영구 누적됩니다.
