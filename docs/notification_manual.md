...
- **타캠핑장 맞춤형 동적 지역코드 오토 맵핑 (Geo-to-RegId Auto Mapping)**:
  사용자가 추가한 '타 캠핑장' 일정의 다양한 위치(제주, 강원 등)에도 완벽한 중기예보를 제공하기 위해, 좌표(lat/lng)를 기상청 전용 **행정구역 코드(예: 11C20000) 및 기온 관측소 코드(예: 11C20401)**로 자동 치환하는 내부 맵핑 로직(`getMidTermRegionCodes`)을 엣지 펑션 레벨에서 동작시킵니다.
  - 전국의 주요 기상청 관측소 16곳과 10개 권역 코드를 하드코딩된 풀로 보유하고, 삼각함수 기반 최단 거리 계산 로직(`calculateDistance`)을 통해 타캠핑장의 실제 좌표와 가장 가까운 관측소 코드를 채택하여 동적으로 기상청 API 파라미터로 넘깁니다.

- **스마트 장소명 렌더링 (Context-Aware Location Display)**:
  - 모든 리마인드 알림의 본문 상단(`📍`)에 해당 일정의 출처(`s.source`)를 분석하여 캠핑장 이름을 똑똑하게 렌더링합니다.
  - 라온아이 예약 건은 '라온아이 캠핑장 (A1 등 사이트명)' 형태로, 사용자가 등록한 타캠핑장 건은 고객이 직접 입력한 이름을 그대로 노출하여 알림 템플릿의 완성도와 몰입감을 높였습니다.
