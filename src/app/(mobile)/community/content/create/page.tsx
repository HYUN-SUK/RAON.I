...
export default function CreatorContentCreatePage() {
    ...
    if (isLoading) {
        return <div className="min-h-screen bg-[#F8FAF8] flex items-center justify-center">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#388E5A]"></div>
        </div>;
    }

    return (
        <div className="pb-24 bg-[#F8FAF8] min-h-screen">
            {/* Simple Header for this sub-page */}
            <header className="sticky top-0 z-50 bg-[#F8FAF8]/80 backdrop-blur-md border-b border-[#ECE8DF] px-4 h-14 flex items-center">
                <button onClick={() => router.back()} className="p-2 -ml-2">
                    <ArrowLeft className="w-6 h-6 text-[#1E4D2B]" />
                </button>
                <h1 className="text-lg font-bold text-[#1E4D2B] ml-2">콘텐츠 발행</h1>
            </header>
