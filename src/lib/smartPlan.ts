...
async function fetchMidpointTrackB(midpoint: {lat: number, lng: number}, weather: string, isWinter: boolean, persona: UserPersona): Promise<FactCard[]> {
    ...
    mergedMap.forEach((row: any) => {
        ...
        const globalBlacklist = /정비|카센터|공업사|세차|타이어|배터리|공인중개사|부동산|장례|상조|종교|교회|사찰$|센터$|학원|관리소|사무소|지물포|건재|상사|유통|공구|이발|미용|세탁|철물|사진관|인쇄소|스튜디오|모텔|여관|호텔|약국|의원|병원|디지털|농약|종묘|정육|방앗간|기름집/;
