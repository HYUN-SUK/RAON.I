...
- [x] **캠핏 [적용] ➔ [확인] 팝업 자동 승인 및 패널 닫기 완결 (2026-08-28 완결)**
    ...
    - [x] 3. 확장프로그램 v2.4.0 릴리즈 & TypeScript 0에러 & Next.js 16 Production Build (`npm run build`) 102개 전 라우트 100% 무결 검증 통과
...
- [x] **물리 좌표(Y축 Geometry) 기반 상단 [적용] 버튼 클릭 완결 (2026-08-28 완결)**
    ...
    - [x] 3. 확장프로그램 v2.2.0 릴리즈 & TypeScript 0에러 & Next.js 16 Production Build (`npm run build`) 102개 전 라우트 100% 무결 검증 통과
...
- [x] **10초 기록 의견수집 2중 유도·나만의 캠핑지도 0초 핀 고속화·PWA 푸시 분기 (2026-08-24 완료)**
    ...
    - [x] 12. 추천 장소 의견수집 화면 UI 전면 개편: 하단 플로팅 고정 바(`fixed bottom-0`, `z-50`) 신설(담기/선택없이 넘어가기 상시 노출) & 우측 상단 '다음에 하기' 선명한 칩 버튼 강화 (`verify/[scheduleId]/page.tsx`)
...
- [x] **라온아이 ↔ 캠핏 양방향 실시간 예약 자동 연동 시스템 구축 (2026-08-23 완료)**
    ...
    - [x] 4. 무한 루프(Echo Loop) 100% 원천 방지: 캠핏 문자 수신 시 `[라온아이]` 태그 감지 시 0초 만에 스킵(`status='SKIPPED_ECHO'`) 처리
    - [x] 5. 사업주 PC 크롬 확장프로그램 실측 로드 및 정상 동작(서비스 워커 활성화) 검증 완료

- [x] **관리자 대시보드 & 결제 목록 UI 전면 개편 (캠핏 결제 관리 스타일 & 오늘 입실팀 모달) (2026-08-23 완료)**
    - [x] 1. 메인 대시보드 '오늘 입실' 카드 클릭 시 오늘 체크인/체류 예약팀 전용 모달(`TodayCheckInsModal`) 팝업 연동
    - [x] 2. 메인 대시보드 카드 개편: '입금 대기' ➔ '결제 목록' 카드로 전환 (오늘 결제총액, 결제대기 건수, 환불대기 건수 통합 표출)
    - [x] 3. 결제 관리 화면(`/admin/payments`) 전면 리뉴얼: 상단 3종 요약 카드(오늘 결제, 환불 대기, 결제 대기) + 6대 상태 탭 바(전체, 결제대기, 결제완료, 환불대기, 환불완료, 취소됨)
    - [x] 4. 검색 및 퀵 기간 필터(오늘~1년) + 10개씩 분할 보기 드롭다운 & 하단 페이지네이션 바(« 1 2 3 ») 구축
    - [x] 5. 최신 변동 시각(`updatedAt || createdAt`) 기준 내림차순 정렬 적용 (방금 취소/환불/예약된 건 최상단 즉시 노출)
    - [x] 6. 결제 목록 행/예약자명 클릭 시 공통 예약 상세 모달(`AdminReservationDetailModal`) 팝업 (입금 확인 및 취소 즉시 액션)
    - [x] 7. 환불/취소 내역에는 예약 취소자의 환불 은행명/계좌번호/예금주명 선명한 표출
    - [x] 8. TypeScript 0에러 및 Next.js 16 Production Build (`npm run build`) 102개 전 라우트 100% 무결 검증 통과

- [x] **A+D 해자 데이터 수집 체계 구축 (Phase 1 ~ Phase 4 전면 완료) (2026-08-22 완료)**
    - [x] 1. Phase 1: DB DDL (6대 해자 테이블 & RLS), 길안내 0ms 무손실 로거, 스마트플랜 스왑/유지 로거 구축
    - [x] 2. Phase 2: 관리자 1단계 팩트 검증 UI(`/admin/verifications`), 활성 11개 카드 1.0 정답셋 적재, 폐업/구제 마스터 동기화
    - [x] 3. Phase 3: 캠핑지도 카테고리 필터 칩 & 5종 SVG 마커, 1초 신고 시트, 길안내 2시간 후 복귀 프롬프트 전역 모달, 독립 라우트(`/verify/[scheduleId]`)
