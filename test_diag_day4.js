    ...
    try {
        ...
        if (profile) {
            const { data: userSchedules } = await supabase.from('user_schedules')
                ...
                .gte('check_in', '2026-02-23') // Look back slightly to ensure we don't miss anything 
                .order('check_in', { ascending: true });

            result.schedules = userSchedules;
        }

        fs.writeFileSync('diag_result_day4.json', JSON.stringify(result, null, 2));
        console.log("Diagnosis complete. Results saved to diag_result_day4.json");
