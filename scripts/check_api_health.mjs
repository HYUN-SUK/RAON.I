...
const API_CONFIGS = [
  ...
  { name: 'TOUR_SPOT', label: '관광명소(TourAPI)', url: `http://apis.data.go.kr/B551011/KorService2/areaBasedList2?serviceKey=${process.env.PUBLIC_DATA_API_KEY}&numOfRows=1&pageNo=1&MobileOS=ETC&MobileApp=RAONAI&_type=json&listYN=Y&arrange=A&contentTypeId=12` },
  { name: 'FESTIVAL', label: '축제(TourAPI)', url: `http://apis.data.go.kr/B551011/KorService2/searchFestival2?serviceKey=${process.env.PUBLIC_DATA_API_KEY}&numOfRows=1&pageNo=1&MobileOS=ETC&MobileApp=RAONAI&_type=json&listYN=Y&arrange=A&eventStartDate=${today}` },
  ...
  { name: 'GOCAMPING', label: '고캠핑', url: `http://apis.data.go.kr/B551011/GoCamping/basedList?serviceKey=${process.env.GOCAMPING_API_KEY}&numOfRows=1&pageNo=1&MobileOS=ETC&MobileApp=RAONAI&_type=json` },
  { name: 'SPOT_TMAP_REL', label: '명소 연관(Tmap)', url: `http://apis.data.go.kr/B551011/TarRlteTarService1/areaBasedList1?serviceKey=${process.env.PUBLIC_DATA_API_KEY}&areaCd=11&signguCd=11110&baseYm=202504&numOfRows=1&_type=json&MobileOS=ETC&MobileApp=RAONAI` },
  { name: 'SPOT_KT_CONCTR', label: '명소 집중률(KT)', url: `http://apis.data.go.kr/B551011/TatsCnctrRateService/tatsCnctrRatedList?serviceKey=${process.env.PUBLIC_DATA_API_KEY}&areaCd=11&signguCd=11110&numOfRows=1&_type=json&MobileOS=ETC&MobileApp=RAONAI` },
  { name: 'KTO_POPULARITY', label: '관광명소(지자체 인기도)', url: `http://apis.data.go.kr/B551011/TarRlteTarService1/areaBasedList1?serviceKey=${process.env.PUBLIC_DATA_API_KEY}&areaCd=34&signguCd=34330&baseYm=202504&numOfRows=1&_type=json&MobileOS=ETC&MobileApp=RAONAI` },
...
async function checkHealth() {
  console.log('🚀 Starting Full API Health Check (v11.9)...');
  ...
  const results = await Promise.all(API_CONFIGS.map(async (api) => {
    ...
    try {
      const response = await fetch(api.url, {
        ...
        body: api.body,
        timeout: api.name === 'LX_RESTAURANT' ? 20000 : 10000
      });
      ...
      if (api.name === 'GEMINI' && !isOk) {
        return {
          name: api.name, label: api.label, status: 'FAILURE', duration_ms: duration,
          error: `HTTP ${response.status} (Deferred)`, checked_at: new Date().toISOString()
        };
      }

      return {
        name: api.name, label: api.label, status: isOk ? 'SUCCESS' : 'FAILURE',
        duration_ms: duration, error: isOk ? '' : `HTTP ${response.status}: ${response.statusText}`,
        checked_at: new Date().toISOString()
      };
    } catch (error) {
      return {
        name: api.name, label: api.label, status: 'FAILURE',
        duration_ms: Date.now() - apiStartTime, error: error.message,
        checked_at: new Date().toISOString()
      };
    }
  }));
